...
const sqlPath = path.join(__dirname, '..', 'web', 'tmp_fase1_ddl_v2.sql');
...
async function run() {
    try {
        ...
        console.log('Script DDL ejecutado con éxito.');
        // console.log(res); // Opcional para ver resultados si son múltiples queries
        
    } catch (err) {
        console.error('Error durante la ejecución del DDL:', err);
    } finally {
        await client.end();
    }
}
